...
public interface SubscriptionRepository extends JpaRepository<Subscription, Integer> {
    ...
    @Query(value = "select * from subscription where class_id = :classId", nativeQuery = true)
    List<Subscription> getSubscriptionsByClass(int classId);

    @Query(value = "select * from subscription where class_id = :classId and user_id = :userId", nativeQuery = true)
